...
import {
  ...
  LEADERBOARD_ROLLOVER_CHECK_SECONDS,
...
import { getLeaderboardWeekStartUtc } from '../shared/leaderboardWeek'
...
let leaderboardWeekStartUtc = getLeaderboardWeekStartUtc()
...
export function setupLeaderboardServer(): Promise<void> {
  ...
  setupLeaderboardRolloverSystem()
...
export async function getPublicLeaderboardSnapshot(): Promise<PublicLeaderboardSnapshot> {
  await setupLeaderboardServer()
  await ensureCurrentLeaderboardWeek()
  return createPublicLeaderboardSnapshot()
}

function createPublicLeaderboardSnapshot(): PublicLeaderboardSnapshot {
  const ranked = getRankedLeaderboardEntries()
  return {
    addresses: ranked.map(([address]) => address),
    names: ranked.map(([, record]) => record.name),
...
export async function getLeaderboardExportSnapshot(): Promise<PublicLeaderboardSnapshot> {
  await setupLeaderboardServer()
  await ensureCurrentLeaderboardWeek()

  const ranked = getAllRankedLeaderboardEntries()
  return {
    addresses: ranked.map(([address]) => address),
    names: ranked.map(([, record]) => record.name),
    scores: ranked.map(([, record]) => record.totalScore),
...
function setupLeaderboardRolloverSystem(): void {
  if (rolloverCheckStarted) return

  rolloverCheckStarted = true
  engine.addSystem((dt) => {
    rolloverCheckAccumulator += dt
    if (rolloverCheckAccumulator < LEADERBOARD_ROLLOVER_CHECK_SECONDS) return

    rolloverCheckAccumulator = 0
    void ensureCurrentLeaderboardWeek().catch((error) => {
      console.log('[Server][LB] Weekly rollover check failed:', error)
    })
  })
}

async function ensureCurrentLeaderboardWeek(): Promise<void> {
  const currentWeekStartUtc = getLeaderboardWeekStartUtc()
  if (currentWeekStartUtc === leaderboardWeekStartUtc) return
  if (rolloverPromise) return rolloverPromise

  rolloverPromise = rolloverLeaderboard(currentWeekStartUtc)
...
async function rolloverLeaderboard(nextWeekStartUtc: string): Promise<void> {
  const previousWeekStartUtc = leaderboardWeekStartUtc
  const previousScores = [...scores.entries()]

  leaderboardWeekStartUtc = nextWeekStartUtc
  scores.clear()

  try {
    await persistToStorage()
  } catch (error) {
    leaderboardWeekStartUtc = previousWeekStartUtc
    scores.clear()
    for (const [address, record] of previousScores) {
  ...
  publishLeaderboard()
  console.log(`[Server][LB] Weekly leaderboard rolled over ${previousWeekStartUtc} -> ${nextWeekStartUtc}.`)
}
...
function getRankedLeaderboardEntries(): [string, PlayerRecord][] {
  return getAllRankedLeaderboardEntries()
    .slice(0, LEADERBOARD_TOP_N)
}

function getAllRankedLeaderboardEntries(): [string, PlayerRecord][] {
  return [...scores.entries()]
    .sort((a, b) => b[1].totalScore - a[1].totalScore)
}

async function loadFromStorage(): Promise<void> {
  leaderboardWeekStartUtc = getLeaderboardWeekStartUtc()
  ...
  const stored = JSON.parse(raw) as Partial<WeeklyLeaderboardStorage>
  if (stored.weekStartUtc !== leaderboardWeekStartUtc || !Array.isArray(stored.entries)) {
    console.log(`[Server][LB] Stored week=${stored.weekStartUtc ?? 'invalid'} expired; starting week=${leaderboardWeekStartUtc}.`)
    await persistToStorage()
    return
  }
...
async function persistToStorage(): Promise<void> {
  try {
    const payload: WeeklyLeaderboardStorage = {
      weekStartUtc: leaderboardWeekStartUtc,
      entries: [...scores.entries()],
    }
    const persisted = await Storage.set(LEADERBOARD_STORAGE_KEY, JSON.stringify(payload))
    if (persisted) return

    throw new Error('Weekly leaderboard was rejected by Storage')
  } catch (error) {
    console.log('[Server][LB] Could not persist weekly leaderboard to Storage:', error)
    throw error
  }
}
