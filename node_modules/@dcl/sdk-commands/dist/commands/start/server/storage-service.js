...
exports.setupStorageEndpoints = setupStorageEndpoints;
 ...
 */
function setupStorageEndpoints(components, router, workspace) {
    const withKeyValidation = async (ctx, next) => {
        if (!ctx.params.key) {
            return { status: 400, body: { message: 'Key is required' } };
        }
        return next();
    };
    const withAddressValidation = async (ctx, next) => {
    ...
    router.get('/env/:key', withKeyValidation, async (ctx) => {
        ...
        const envVars = await (0, runtime_env_1.getMergedEnv)(components, workspace.projects[0].workingDirectory);
    ...
    router.put('/env/:key', withKeyValidation, async (ctx) => {
        ...
        try {
            ...
            await (0, runtime_env_1.setEnvValue)(components, key, value);
            return { status: 204 };
        }
        catch (error) {
    ...
    router.delete('/env/:key', withKeyValidation, async (ctx) => {
        const { key } = ctx.params;
        try {
            await (0, runtime_env_1.deleteEnvValue)(components, key);
            return { status: 204 };
        }
        catch (error) {
    ...
    router.get('/values', async (ctx) => {
        ...
        const storage = await (0, runtime_env_1.loadServerStorage)(components);
    ...
    router.get('/values/:key', withKeyValidation, async (ctx) => {
        const { key } = ctx.params;
        const value = await (0, runtime_env_1.getWorldValue)(components, key);
        if (value === undefined) {
    ...
    router.put('/values/:key', withKeyValidation, async (ctx) => {
        ...
        try {
            const bodyText = await ctx.request.text();
            const { value } = JSON.parse(bodyText);
            await (0, runtime_env_1.setWorldValue)(components, key, value);
            return { body: JSON.stringify({ value }) };
        }
        catch (error) {
    ...
    router.delete('/values/:key', withKeyValidation, async (ctx) => {
        const { key } = ctx.params;
        try {
            await (0, runtime_env_1.deleteWorldValue)(components, key);
            return { status: 204 };
        }
        catch (error) {
    ...
    router.get('/players/:address/values/:key', withAddressValidation, withKeyValidation, async (ctx) => {
        ...
        const value = await (0, runtime_env_1.getPlayerValue)(components, address, key);
        if (value === undefined) {
    ...
    router.put('/players/:address/values/:key', withAddressValidation, withKeyValidation, async (ctx) => {
        ...
        try {
            const bodyText = await ctx.request.text();
            const { value } = JSON.parse(bodyText);
            await (0, runtime_env_1.setPlayerValue)(components, address, key, value);
            return { body: JSON.stringify({ value }) };
        }
        catch (error) {
    ...
    router.delete('/players/:address/values/:key', withAddressValidation, withKeyValidation, async (ctx) => {
        const { address, key } = ctx.params;
        try {
            await (0, runtime_env_1.deletePlayerValue)(components, address, key);
            return { status: 204 };
        }
        catch (error) {
