...
async function setupEcs6Endpoints(components, router, workspace) {
    ...
    router.all('/lambdas/:path+', async (ctx) => {
        ...
        return {
            ...
            body: req.body ? stream_1.Readable.fromWeb(req.body) : undefined
        };
    });
    router.post('/content/entities', async (ctx) => {
        ...
        return {
            status: res.status,
            headers,
            body: res.body ? stream_1.Readable.fromWeb(res.body) : undefined
        };
    });
...
async function serveWearable(components, project, baseUrl) {
    ...
    const wearableId = wearableCache.get(sceneHash) ?? `urn:${(0, crypto_1.randomUUID)()}`;
