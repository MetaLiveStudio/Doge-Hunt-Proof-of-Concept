...
|---|---|---|---|---|
| 2026-09-05 | Remove local and generated project files from Git | Ready for review | Codex | Excluded dependencies, Creator Hub editor state, AI reference context, generated thumbnails, skill lock, empty scene README, and local npm wrapper. No target paths remain tracked; 18,582 `node_modules` files were removed from the index. Runtime source/assets, `main.crdt`, and debug documentation remain. README now explains the dependency and local-file boundary. Staged and unstaged diff checks pass. |
| 2026-09-05 | Exclude the generated Codex scene duplicate from Git | Ready for review | Codex | `.codex-build-hint-white-outline/` is an unreferenced full scene duplicate. Added `.codex-*/` ignore coverage and removed its tracked copy from Git without deleting local files. Ignore verification passes. |
...
| 2026-09-05 | Refresh README for the authoritative multiplayer release | Ready for review | Codex | Replaced the obsolete mobile-only README with the current room flow, authoritative BONK model, spectator rules, leaderboard/points, desktop/mobile details, project map, commands, and manual Preview checklist. The document distinguishes build evidence from manual Creator Hub and multiplayer acceptance. Markdown diff checks pass. |
| 2026-08-30 | Number the white lobby How to Play steps | Ready for desktop/mobile Preview | Codex | Prefixed the three white lobby instructions with `1/`, `2/`, and `3/`; the gold win condition remains unnumbered. TypeScript and targeted diff checks pass. |
| 2026-08-30 | Combine the anonymous-Doge lobby instruction | Ready for desktop/mobile Preview | Codex | Merged `All Doges look identical.` and the BONK instruction into one auto-wrapping lobby How to Play paragraph. The black panel remains content-height-driven: desktop retains its 560px scaled layout, while mobile retains its 320px ScreenInsetArea-safe layout and independent padding, so both backgrounds resize with the new paragraph rather than preserving the old two-line gap. TypeScript, SDK build, and targeted diff checks pass. |
| 2026-08-30 | Clarify the solo-confirmation action | Ready for desktop/mobile Preview | Codex | The affirmative solo-confirmation action now reads `YES, I NEED PRACTICE` and uses the exact same compact/desktop font size as `NO, I'LL WAIT FOR OTHERS`. Behavior and layout are unchanged. TypeScript and targeted diff checks pass. |
| 2026-08-30 | Place waiting-room guidance below its actions | Ready for desktop/mobile Preview | Codex | Moved the shared Waiting Room guidance line below the primary/secondary action buttons, with a fixed gap. This covers solo/party host guidance, participant Ready/waiting guidance, and the match-start countdown. Player-row `HOST`/`READY`/`WAITING` labels and the blue Room entry connection status remain in place. TypeScript and targeted diff checks pass. |
| 2026-08-30 | Hide room snapshot versions from player-facing labels | Ready for desktop/mobile Preview | Codex | Removed `v31`-style room snapshot versions from room-entry status (`Room open` / `Room ready`) and the 3D lobby prompt (`Room open 1/10` / `Room ready 1/10`). Client/server snapshot logs still include `version` for diagnostics. TypeScript, targeted diff, and visible-version scan pass. |
| 2026-08-30 | Cancel a match countdown without leaving the room | Ready for desktop/mobile Preview | Codex | Countdown `CANCEL` now sends a dedicated authoritative cancellation request instead of leaving the room. Only the host that initiated the pending countdown may cancel it; the server clears the pending start and broadcasts a waiting-room snapshot, keeping every player in the same room. Guests see the control disabled during countdown. TypeScript, SDK build, and targeted diff checks pass. Preview a host-started countdown with two clients: host presses Cancel and both clients should return to Waiting Room with the same players still present. |
| 2026-08-30 | Refine solo and party host guidance | Ready for desktop/mobile Preview | Codex | Solo host guidance is now `You are the host. Start the game when everyone is ready, or play solo.` Party host guidance is `You are the host. Start the game when everyone is ready.` No layout, button, or server behavior changed. TypeScript and targeted diff checks pass. |
| 2026-08-30 | Unify host waiting-room labels | Ready for desktop/mobile Preview | Codex | Solo and party hosts now both see `START GAME` and `You are the host, start game when everyone is ready`. A solo host still receives the separate solo confirmation after pressing Start Game; no server or readiness behavior changed. TypeScript and targeted diff checks pass. |
| 2026-08-30 | Make the waiting-room host the explicit game starter | Ready for desktop/mobile Preview | Codex | In a party room, the host now sees `You are the host, start game when everyone is ready` and `START GAME`, rather than an individual Ready action. The server now regards a party as eligible when every non-host player is ready; the host remains the only account allowed to start or maintain the countdown. Host rows display `HOST`, not a misleading waiting state. Solo confirmation remains unchanged. TypeScript, SDK build, and targeted diff checks pass. Preview desktop/mobile with host plus one guest: guest becomes Ready, host Start Game enables, then verify the shared 3-second countdown; verify it is disabled while the guest is unready. |
| 2026-08-30 | Confirm solo starts from the Waiting Room | Ready for desktop/mobile Preview | Codex | New room entrants remain unready. A solo host now sees `PLAY SOLO` instead of an extra `I'M READY` step; tapping it opens `Are you sure that you wanna play solo?` with `YES` and `NO, I'LL WAIT FOR OTHERS`. `YES` synchronizes Ready first, then starts only after the server snapshot confirms solo eligibility; `NO` leaves the room/player readiness unchanged. The dialog closes if a player joins while it is open. Party Ready/start, countdown cancel, and server authority are unchanged. TypeScript, SDK build, and targeted diff checks pass. Preview desktop and mobile: create a solo room, choose No, then choose Yes and verify the 3-second server countdown; join a second player while the dialog is open and confirm it closes. |
| 2026-08-30 | Use authoritative BONK total in HUD and results | Ready for desktop/mobile Preview | Codex | The HUD and results header now prefer the server-confirmed local player's BONK total, which includes accepted NPC and real-player BONKs. Local presentation counting remains only as a fallback before the relevant server snapshot arrives. This fixes the observed disagreement where the server-backed ranking row displayed `1 Bonks` but the personal header displayed `0`. TypeScript, SDK build, and targeted diff checks pass. Preview one confirmed NPC BONK and verify the top HUD/results total matches the player row. |
